...
const NightModeToggle = () => (
  <ThemeToggler>
    {({theme, toggleTheme}) => (
      <div className={`absolute m-8 top-0 right-0 border rounded-full border-grey flex items-center cursor-pointer w-15 bg-black ${theme === 'dark' ? 'justify-end' : 'justify-start'}`} onClick={() => toggleTheme(theme === 'light' ? 'dark' : 'light')} role="checkbox" aria-checked={theme === 'dark'}>
